# scraper.py (Perbaikan Terakhir untuk Mengatasi Semua Error)
...
def get_google_trends_score(keyword: str) -> int:
    try:
        ...
        # Hapus argumen 'method_whitelist' jika ada
...
def get_shutterstock_competition_selenium(keyword: str) -> int:
    print(f"Menganalisis Shutterstock untuk '{keyword}' menggunakan Selenium...")
    ...
    # Menentukan lokasi biner Chrome
    chrome_options.binary_location = '/usr/bin/google-chrome-stable'
    
    # Menentukan lokasi chromedriver secara manual
    ...
    try:
        ...
        result_element = driver.find_element(By.XPATH, "//*[contains(text(), 'stock images')]")
        
        result_text = result_element.text
        number_str = ''.join(filter(str.isdigit, result_text.split("stock images")[0]))
        ...
        print(f"Peringatan: Elemen jumlah kompetisi tidak ditemukan di halaman untuk '{keyword}'.")
        return 0
    except Exception as e:
        print(f"Error saat scraping Selenium ke Shutterstock untuk '{keyword}': {e}")
        return 0
    finally:
        if driver:
            driver.quit()
